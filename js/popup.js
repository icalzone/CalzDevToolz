...
document.addEventListener('DOMContentLoaded', function() {
    chrome.tabs.query({'active': true, 'lastFocusedWindow': true}, function (tabs) {
        var curPage = tabs[0].url.split('?')[0];
        var match = curPage.match(/(\/en-US\/|\/es\/|\/zh\/|\/pt\/|\/ru\/|\/de\/|\/fr\/|\/it\/|\/ko\/|\/pl\/)/);
        if (match.length > 0){
            curPage = curPage.replace(match[0],'/');
        };

        curPage += "?region=EMEA&epslanguage=en";
        curPage = encodeURI(curPage);

        $("a.addUrl").each( function(){
            var url = $(this).attr("href");
            url = url.replace("{URL}", curPage)
            $(this).attr("href",url);
        });
